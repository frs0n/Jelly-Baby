...
- The swing and trampoline merged from upstream are shared, single-occupancy world objects. The server owns occupancy: it validates that a player is standing on the ground within reach of the anchor, refuses a facility that already has a rider, seats the rider on the anchor, and places them on the clear approach side when they step off. A rider is excluded from the player collision pass and can neither grab nor be grabbed. The ride itself is simulated by its rider and published as one scalar — the swing's seat angle, or the trampoline's centre height above the bed — clamped by the authority and carried in the existing delta stream at 0.1 mrad and 0.1 mm. Mounting is predicted locally and rolled back if the authority refuses the seat, in the same way dragging another player already is; server reconciliation is suspended for the rider only while the facility owns their body.
- Observers reproduce a ride from that one scalar. The remote actor runs the same seat springs and the same FEM the rider runs, so an observed swing sweeps its real arc and an observed bounce follows the relayed height, with the wobble coming from the solver rather than from replayed vertices. The shared swing and trampoline visuals, and their sound, are driven from the interpolated phase of whoever the authority says is riding, so every client draws the same seat at the same angle. Facility shadows and the laughing expression are unchanged from upstream.
...
npm run test:facility-online
npm run test:multitouch
npm run test:performance
```

`test:online` verifies input, collisions, jumping, dash cooldowns, sustained six-player stability, and deterministic colours. `test:rooms` verifies concurrent joins, capacity, real WebSocket broadcasting, a visible facility ride end to end — mounting, exclusive occupancy, the relayed phase reaching another client, and the dismount position — reuse after leaving, rejected seat replays, colour privacy, forged facility ids, and the absence of chat in Miniflare/workerd. Room tests use temporary local storage and never access online players. `test:swing`, `test:trampoline`, `test:facility-shadows` and `test:facility-sound` are the upstream facility suites and run unchanged here. `test:facility-online` covers what this fork adds to them: command validation, exclusive occupancy, grab exclusion, phase clamping, a rider's immunity to the collision solver, delta encoding and quantisation of the two new fields, and an observer reproducing both rides from the relayed scalar before handing back to position reconciliation.
