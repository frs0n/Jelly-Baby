...
- 服务器 30 Hz 调度、每次 3 个碰撞子步运行简化碰撞体，常规运动最多 15 Hz 广播增量状态，抓取开始/结束即时广播，静止状态每秒保活一次。入场发送身份/外观基线，此后用房间内短编号、字段位掩码和 0.1 mm 量化整数，只发送有变化的字段（不发送抓取保活时间）；移动方向归一化，跳跃与冲撞冷却由服务器决定。碰撞体不是逐顶点软体碰撞。
- 本地保留即时软体运动，只平滑纠正明显的服务器位置偏差；小误差和静止落地高度不会唤醒软体或触发光学重算。利用测得的往返时间补偿快照延迟，远端角色使用约 100 毫秒的快照插值缓冲。
- 本机角色保持完整皮肤和 240 Hz FEM；远端角色用已有的约 2 万三角形表面（原约 14.4 万）和 120 Hz FEM，保留步态、表情及软体约束。远端网格最多 30 Hz 更新，位置/朝向/抓取目标插值与屏幕刷新解耦。每个角色在浏览器本地 Web Worker 中计算，最多一个在途请求，复用可转移缓冲区；主线程更新网格并剔除视野外对象。其他角色仍未单独运行光学追踪。抓取绑定实际皮肤，拉别人时伸出自己原有的手，连接被抓皮肤的实际位置；松手撤销约束。拖动他人的皮肤及伸手先在本地预测，不等待服务器往返；服务器校验距离与所有权，拒绝时撤销预测，限制拉力并保留释放动量。移动/拖动目标不变时只每 200 ms 保活一次，停止、跳跃及释放不会被去重吞掉。自动配色使用明亮的相近色渐变，保留原有着色器与吸收计算。
- 握手座位 20 秒过期且只能使用一次；连接有消息大小、速率限制和同源检查。移动输入超过 350 毫秒未更新就停止移动（心跳和拖动消息不能续期移动输入），失联 15 秒释放座位。空桌停止计时器。断线明确提示重新加入，不切换成假联机或单人模式。
...
多人性能回归：`test:protocol` 检查六人移动/拖动的增量编解码、成员离场/编号复用、基线与历史快照不可变性，测试轨迹数据量比完整 JSON 快照减少约 86%；`test:network-budget` 检查输入去重、及时停止/释放、本地预测与 60/120/144 Hz 屏幕下有界 Worker 调度；`test:remote` 包含五个 Worker 同时拖动、释放和可转移缓冲区复用。网格顶点始终仅在浏览器内部传递，不经过服务器。
