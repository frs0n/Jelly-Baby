import { pull, releaseGrabs, type Grip } from './grabs.ts';
import type { Appearance } from './appearance.ts';
import { FACILITY_NONE, clampPhase, facilityAnchor } from './facility-state.ts';
...
export type Controls={x:number;z:number;jump:boolean;dash:boolean;phase:number};
export type Impact={seq:number;nx:number;nz:number;speed:number;height:number;age:number};
export type Player={id:string;x:number;y:number;z:number;vx:number;vy:number;vz:number;yaw:number;appearance:Appearance;cooldown:number;dashTime:number;hit:number;impacts:Impact[];grab:Grip|null;facility:number;phase:number};
...
export function spawn(id:string,others:Player[],appearance:Appearance={primary:'#b6e778',secondary:'#b6e778',angle:0}):Player {
  ...
  return {id,x,y:0,z,vx:0,vy:0,vz:0,yaw:Math.atan2(-x,-z),appearance,cooldown:0,dashTime:0,hit:0,impacts:[],grab:null,facility:FACILITY_NONE,phase:0};
}
export function controls(value:unknown):Controls|null {
  ...
  if(v.phase!==undefined&&(typeof v.phase!=='number'||!Number.isFinite(v.phase)))return null;
  const n=Math.max(1,Math.hypot(v.x,v.z));
  return {x:v.x/n,z:v.z/n,jump:v.jump,dash:v.dash,phase:v.phase===undefined?0:v.phase as number};
}

/** Seat a player, if the facility is free and they are standing beside it.
 * Occupancy is authoritative so two players can never share one swing; the
 * ride itself is simulated by the rider and relayed as a single phase. */
export function enterFacility(players:Player[],id:string,facility:number) {
  const anchor=facilityAnchor(facility),p=players.find(p=>p.id===id);
  if(!anchor||!p||p.facility!==FACILITY_NONE)return false;
  if(p.grab||players.some(other=>other.grab?.by===id))return false;
  if(p.y>.004||Math.hypot(p.x-anchor.x,p.z-anchor.z)>anchor.reach)return false;
  if(players.some(other=>other.facility===facility))return false;
  releaseGrabs(players,id);
  p.facility=facility;p.phase=0;
  p.x=anchor.x;p.z=anchor.z;p.y=0;p.vx=p.vy=p.vz=0;
  return true;
}

/** Step off onto the clear approach side, matching the rider's own dismount. */
export function leaveFacility(players:Player[],id:string) {
  const p=players.find(p=>p.id===id);
  if(!p||p.facility===FACILITY_NONE)return false;
  const anchor=facilityAnchor(p.facility)!;
  p.x=anchor.x+anchor.exit.x;p.z=anchor.z+anchor.exit.z;p.y=0;p.vx=p.vy=p.vz=0;
  p.facility=FACILITY_NONE;p.phase=0;
  return true;
}
...
function step(players:Player[],inputs:Map<string,Controls>,dt:number) {
  for(const p of players) {
    const c=inputs.get(p.id)??{x:0,z:0,jump:false,dash:false,phase:0};
    ...
    if(p.facility!==FACILITY_NONE) {
      // The facility owns a rider's pose, so locomotion and gravity step aside.
      // Parking the collision body on the anchor keeps walkers, grabs and every
      // observer's world agreeing on where the rider is.
      const anchor=facilityAnchor(p.facility)!;
      p.phase=clampPhase(p.facility,c.phase);
      p.x=anchor.x;p.z=anchor.z;p.y=0;p.vx=p.vy=p.vz=0;p.grab=null;
  ...
  for(let pass=0;pass<2;pass++)for(let i=0;i<players.length;i++)for(let j=i+1;j<players.length;j++) {
    ...
    // Riders are held clear of the floor by the facility, not by the solver.
    if(a.facility!==FACILITY_NONE||b.facility!==FACILITY_NONE)continue;
