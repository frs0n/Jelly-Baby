...
export class Locomotion {
  ...
  reset() { this.yaw=0; this.phase=0; this.jumpQueued=false; this.move.set(0,0,0); }
  step(h:number) {
    ...
    for(let i=0;i<b.mass.length;i++) {
      ...
      if(x[j+1]<.0018) groundedMass+=w;
    }
    this.grounded=groundedMass>.004;
    ...
    this.phase+=h*Math.min(1,this.velocity.length()/.10)*14;
    ...
    for(let i=0;i<b.mass.length;i++) {
      ...
      const arm=Math.max(0,(Math.abs(rx)-.020)/.014);
      const stride=Math.sin(this.phase+(rx<0?0:Math.PI))*Math.min(1,speed+.2);
